...
import { getAllApps, getNumericDownloads, APP_CATEGORIES, formatPlayStoreDownloads, isPremiumApp, getStoreState } from '../data';
import { Crown, ArrowRight, LayoutGrid, List, ArrowDownToLine, ChevronLeft, ChevronRight, SlidersHorizontal } from 'lucide-react';
...
export default function AppsPage() {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid'); // Changed to grid by default
  ...
  useEffect(() => {
    document.title = "Download Free Android Apps - RapidAPK";
    ...
    metaDescription.setAttribute("content", "Browse and download the best free Android apps and utilities rapidly and securely on RapidAPK.");
    ...
    metaKeywords.setAttribute("content", "android apps, download apps, free android apps, best apps, apk");
  }, []);
  ...
  useEffect(() => {
    ...
    // Categorize as apps (anything that isn't a game or explicitly has apps/default category)
    const appsList = all.filter(app => {
      const isApp = app.category === 'apps' || !app.category;
      return isApp;
    });
    
    setAllApps(appsList);
    
    // Extract "Top Apps" - sorted by score of downloads and rating
    const sortedForTop = [...appsList].sort((a, b) => {
    ...
    // Top 6 are featured
    setTopApps(sortedForTop.slice(0, 6));
  }, []);
  ...
  }, [allApps, selectedTag, searchQuery, sortBy]);
  ...
  const handlePageChange = (pageNumber: number) => {
    if (pageNumber >= 1 && pageNumber <= totalPages) {
      ...
      // Beautiful smooth scroll to the main section
      const contentSection = document.getElementById('apps-explore-section');
      if (contentSection) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 dark:bg-slate-900 pb-safe animate-in fade-in duration-300">
      ...
      <div className="bg-[#1e293b] text-white py-12 px-6 shadow-inner relative overflow-hidden">
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-gradient-to-br from-indigo-500/20 to-emerald-500/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none" />
        <div className="absolute bottom-0 left-0 w-80 h-80 bg-blue-500/10 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2 pointer-events-none" />
        
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
          <div>
            <div className="inline-flex items-center gap-2 bg-indigo-500/20 border border-indigo-400/30 px-3 py-1 rounded-full text-indigo-300 text-[10px] font-black uppercase tracking-widest mb-3">
              Premium Application Portal
            </div>
            <h2 className="text-4xl md:text-5xl font-black tracking-tight leading-none mb-3">
              Android <span className="text-lite">Apps</span>
            </h2>
            <p className="text-slate-300 text-sm max-w-[480px] leading-relaxed font-medium">
              Explore safe, ad-free and feature-rich premium APK collections with professional modifications and editor checks.
            </p>
          </div>
      ...
      {/* 👑 Top Apps Section */}
      {topApps.length > 0 && (
        <section className="bg-white dark:bg-slate-950 py-8 border-b border-gray-100 dark:border-slate-800 dark:border-slate-800">
          <div className="max-w-7xl mx-auto px-5 sm:px-8">
            <div className="flex items-center gap-2.5 mb-6">
              <div className="w-9 h-9 bg-amber-50 dark:bg-amber-900/30 rounded-xl flex items-center justify-center border border-amber-100 dark:border-amber-800 shadow-sm">
                <Crown size={18} className="text-amber-500 fill-amber-500" />
              </div>
              <div>
                <h3 className="font-extrabold text-[22px] tracking-tight text-gray-900 dark:text-gray-100 leading-none">Top Apps</h3>
                  ...
                  >
                    <div className="relative aspect-square rounded-2xl overflow-hidden bg-white dark:bg-slate-900 border border-gray-150 dark:border-slate-800 shadow-sm flex items-center justify-center group-hover:scale-[1.03] transition-transform">
                      ...
                      <div className="absolute bottom-1.5 left-1.5 flex flex-col z-10 w-full mb-1">
                           ...
                           <span className="bg-[#6ab344] text-white text-[6px] font-black px-1.5 py-0.5 mt-[1px] w-fit rounded-sm shadow-sm">
                             PREMIUM
                           </span>
                      </div>
                    </div>
                    ...
                    <div className="min-w-0 px-1">
                      ...
                      <p className="text-[11px] text-gray-400 font-medium truncate mt-0.5">
                        {app.subtitle || app.tag || 'Application'}
                      </p>
                    </div>
                  </div>
                );
      ...
      <section id="apps-explore-section" className="max-w-7xl mx-auto w-full px-5 sm:px-8 py-8 flex-1">
              ...
              <button
                ...
                className={`px-4 py-2 text-xs font-black tracking-wide rounded-full border transition-all shrink-0 uppercase ${
                  isActive
                    ...
                    : 'bg-white dark:bg-slate-950 text-gray-500 dark:text-gray-300 border-gray-200 dark:border-slate-700 dark:border-slate-700 hover:border-lite hover:text-lite'
                }`}
              >
                {tag}
              </button>
          ...
          <div>
            <h3 className="font-extrabold text-[20px] text-gray-900 dark:text-gray-100 tracking-tight flex items-center gap-2">
              ...
              <span className="text-xs font-black bg-gray-200/70 text-gray-600 dark:text-gray-400 px-2.5 py-1 rounded-full uppercase">
                {filteredApps.length.toLocaleString()} Apps
              </span>
            </h3>
          </div>
        ...
        {storeState.siteSettings?.adPlacements?.appsPage ? (
          <div 
             className="w-full flex flex-col items-center justify-center mb-6 max-w-2xl mx-auto overflow-hidden" 
             dangerouslySetInnerHTML={{ __html: storeState.siteSettings.adPlacements.appsPage }} 
          />
        ) : (
          <div className="w-full bg-[#eeeeee] flex flex-col items-center justify-center p-8 rounded-xl mb-6 shadow-sm border border-gray-100 dark:border-slate-800 max-w-2xl mx-auto h-[200px]">
             <span className="text-sm font-bold text-gray-500 mt-auto bg-white dark:bg-slate-950/50 px-4 py-1.5 backdrop-blur-sm rounded-lg mb-2">Discover Hidden Gems (Placeholder)</span>
          </div>
        )}
        ...
        {currentItems.length === 0 && (
          <div className="bg-white dark:bg-slate-950 border border-gray-150 rounded-2xl p-12 text-center shadow-sm max-w-lg mx-auto mt-6">
            ...
            <h4 className="font-black text-lg text-slate-800 dark:text-slate-200 tracking-tight">No Apps Found</h4>
            <p className="text-slate-400 text-xs mt-2 font-medium max-w-[340px] mx-auto leading-relaxed">
              We couldn't find any premium applications matching "{searchQuery}" or the "{selectedTag}" category. Try adjusting your filters.
            </p>
            <button 
              ...
              return (
                ...
                >
                  ...
                  <div className="w-full aspect-square bg-[#f8f9fa] dark:bg-slate-900 border border-gray-100 dark:border-slate-800/50 rounded-[14px] overflow-hidden relative shadow-sm shrink-0 group-hover:border-lite/40 transition-colors flex items-center justify-center mb-2.5">
                    {/* Dynamic Overlay tags top-left corner */}
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-between px-0.5">
                    ...
                    <div className="flex items-center gap-1.5 mt-2 text-[11px] font-bold text-orange-500 truncate">
                      <span className="flex items-center gap-0.5 text-[#db7f36]"><Star size={10} className="fill-[#db7f36]" /> {app.rating || '4.5'}</span>
